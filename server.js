...
// pool.connect()
// // .then(function () {
// //     console.log(`Connection made to database "${env.database}"`);
// // });
...
app.get("/satdata/tle", function (req, res) {
    ...
    for (let mainCounter = 0; mainCounter < noradIdList[launchGroup].length; mainCounter++) { 
    // loop for api call for each satellite's TLE data
        ...
        axios
            ...
            .then(function (response) {
                ...
                let satRecord = satellite.twoline2satrec(tleArray[0], tleArray[1]);
                satRecordList[satName] = satRecord;

                let julianSatEpoch = satRecord.jdsatepoch;
                let julianSatEpochMilliSecond = (julianSatEpoch - 2440587.5) * 86400000;
                let satEpochTime = new Date(julianSatEpochMilliSecond);

                let satAttributes = getSatelliteLocation(satEpochTime, satRecord);
                ...
                if (satAttributeList.length == noradIdList[launchGroup].length) {
                    // console.log(satAttributeList);
                    // console.log(satJson);
                    // mapJson = satJson;
                    
                    satAttributeList.sort((a, b) => parseFloat(a.id) - parseFloat(b.id))
...
app.get("/satdata/pos", function (req, res) {
    ...
    axios
        .get(`${baseUrl}positions/${noradId}/${userLat}/${userLong}/${userAlt}/${orbitTime}&apiKey=${apiKey}`)
        .then(function (response) {
            ...
            for (let orbitTimeCounter = 0; orbitTimeCounter < orbitTime; orbitTimeCounter++) {
                ...
                let satlatitude = rawOrbitTrackList[orbitTimeCounter]['satlatitude'];
                let satlongitude = rawOrbitTrackList[orbitTimeCounter]['satlongitude'];
                let sataltitude = rawOrbitTrackList[orbitTimeCounter]['sataltitude'];
            ...
            if (rawOrbitTrackList.length == orbitTrackArray.length) {
                ...
                // sort by timestamp
                orbitTrackArray.sort((a, b) => parseFloat(a.timestamp) - parseFloat(b.timestamp)) 
